...
(function (factory) {
  ...
  var RangeSlider = /*#__PURE__*/function () {
    function RangeSlider($component) {
      ...
      this.suffix = this.$component.dataset.suffix;
      this.$slider = this.$component.querySelector('.range-slider__target');
      this.inputs = [this.$component.querySelector('.range-slider__value-start'), this.$component.querySelector('.range-slider__value-end')]; //this.btnReset = this.$component.querySelector('.range-slider__reset');
      //this.addEventHandlers();

      this.render(); //this.initInputs();
    } // addEventHandlers() {
    // 	if (this.btnReset) {
    ...
    _createClass(RangeSlider, [{
      ...
      value: function render() {
        ...
        this.$slider.noUiSlider.on('update', function (values, handle) {
          ...
          _this4.inputs[handle].dispatchEvent(new Event('input'));
        });
      }
    }]);
    ...
    _createClass(QuantityInput, [{
      ...
      value: function render() {
        ...
        this.$minusButton.onclick = function () {
          _this5.$input.value = _this5.$input.value > _this5.min ? parseInt(_this5.$input.value) - 1 : _this5.$input.value;
        };

        this.$plusButton.onclick = function () {
          if (_this5.max) {
            _this5.$input.value = _this5.$input.value < _this5.max ? parseInt(_this5.$input.value) + 1 : _this5.$input.value;
          } else {
            _this5.$input.value = parseInt(_this5.$input.value) + 1;
          }
        };
      }
    }]);
  ...
  var Dom7 = /*#__PURE__*/function (_Array) {
    ...
    function Dom7(items) {
      ...
      if (typeof items === 'number') {
        _this6 = _super.call(this, items);
      } else {
        _this6 = _super.call.apply(_super, [this].concat(_toConsumableArray(items || [])));
        makeReactive(_assertThisInitialized(_this6));
      }
  ...
  function loopCreate() {
    ...
    for (var _i4 = 0; _i4 < appendSlides.length; _i4 += 1) {
      $selector.append($$1(appendSlides[_i4].cloneNode(true)).addClass(params.slideDuplicateClass));
    }

    for (var _i5 = prependSlides.length - 1; _i5 >= 0; _i5 -= 1) {
      $selector.prepend($$1(prependSlides[_i5].cloneNode(true)).addClass(params.slideDuplicateClass));
    }
  }
  ...
  */
  ...
  function initSliders() {
    ...
    if (document.querySelector('.rs-slider__slider')) {
      new Swiper('.rs-slider__slider', {
        ...
        loop: false,
        navigation: {
          nextEl: '.rs-slider__slider .slider-arrow_next',
          prevEl: '.rs-slider__slider .slider-arrow_prev'
        },
        pagination: {
          el: '.rs-slider__slider .slider-paging',
          type: 'bullets',
          clickable: true
        },
        effect: 'fade',
    ...
    var goodsSliders = document.querySelectorAll('.goods-slider');
    goodsSliders.forEach(function (node, _) {
      new goodsSlider(node);
    }); //thumbs sliders
    //same sliders

    var productSliders = document.querySelectorAll('.product-slider');
    productSliders.forEach(function (node, _) {
      new productSlider(node);
    });
  } //same sliders	objects ==================================================================================================


  var goodsSlider = /*#__PURE__*/function () {
    function goodsSlider($component) {
      _classCallCheck(this, goodsSlider);
      ...
      this.$paging = this.$component.querySelector('[class*="-paging"]');
      this.render();
    }

    _createClass(goodsSlider, [{
      key: "render",
      value: function render() {
  ...
  var productSlider = /*#__PURE__*/function () {
    function productSlider($component) {
      _classCallCheck(this, productSlider);

      this.$component = $component;
      this.$mainSlider = this.$component.querySelector('.product-slider-main');
      this.$thumbsSlider = this.$component.querySelector('.product-slider-thumbs');
      this.$arrowNext = this.$component.querySelector('[class*="_next"]');
      this.$arrowPrev = this.$component.querySelector('[class*="_prev"]');
      this.$paging = this.$component.querySelector('[class*="__paging"]');
      this.render();
    }
    ...
    _createClass(productSlider, [{
      key: "render",
      value: function render() {
        this.thumbs = new Swiper(this.$thumbsSlider, {
          modules: [Thumb],
          spaceBetween: 4,
          slidesPerView: 5,
          freeMode: true,
  ...
  function stickyHeaderAction(e) {
    ...
    if (window.pageYOffset > offset && scrolled > 130) {
  ...
  /safari/.test(getBrowser$1().toLowerCase()); // if (is_safari) {
  ...
  window.addEventListener('load', function (e) {
    ...
    document.body.classList.add('loaded'); // $('.counter').counterUp({
    // 	delay: 10,
    // 	time: 1000,
    // });
    ...
    $('.category-sort__item').on('click', function (event) {
      ...
      if ($(window).innerWidth() < 991) {
        $('.category-sort__list').removeClass('active');
      }
    }); // add other links for tabs

    $('[data-tabs-link]').click(function () {
      $('.product-tabs__nav li').removeClass('active');
      var link = $(this).data('tabs-link');
      $(".product-tabs__nav a[href=\"".concat(link, "\"]")).parent().addClass('active');
    ...
    $('.catalog-menu .catalog-menu__list .catalog-menu-btn').click(function (event) {
      event.preventDefault();

      if ($(this).hasClass('dropdown-is-open')) {
        $(this).toggleClass('dropdown-is-open');
        $(this).next().toggleClass('is-open').slideToggle('slow');
      } else {
        $('.catalog-menu .catalog-menu__list .catalog-menu-btn').removeClass('dropdown-is-open');
        $('.catalog-menu .catalog-menu__list .is-open').removeClass('is-open').slideToggle('slow');
        $(this).addClass('dropdown-is-open');
        $(this).next().addClass('is-open').slideToggle('slow');
      }
    });
    $('.catalog-menu .catalog-menu__label').click(function () {
      $(this).toggleClass('active');
      $(this).next().slideToggle('slow');
    }); //category catalog slide toggle
    // let categoryCatalogItems = document.querySelectorAll('.category-catalog .category-catalog__item');
    // console.log(categoryCatalogItems)
    // if (categoryCatalogItems.length > 8) {
    // 	categoryCatalogItems.forEach( (node, _) => {
    // 		console.log(node);
    // 	});
    // }

    $('.category-catalog .category-catalog-btn').click(function () {
      $(this).prev().slideToggle('slow');

      if ($(this).hasClass('show')) {
        $(this).html('Скрыть').removeClass('show');
      } else {
        $(this).html('Показать всё').addClass('show');
      }
    });
  });
});