...
(function (factory) {
  ...
  if (fileUpload) {
    var fileLabel = fileUpload.querySelector('label');
    fileUpload.querySelector('input').onchange = function () {
      var fileName = '';
      if (this.files && this.files.length > 1) fileName = 'Выбрано ' + this.files.length + ' файла(ов)';else fileName = this.value.replace(/C:\\fakepath\\/i, '');
      if (fileName) {
  ...
  var ItcCustomSelect = /*#__PURE__*/function () {
    function ItcCustomSelect(target, params) {
      ...
      this.EL = 'itc-select';
      this.EL_SHOW = 'itc-select_show';
      this.EL_OPTION = 'itc-select__option';
      this.EL_OPTION_SELECTED = 'itc-select__option_selected';
      this.DATA = '[data-select]';
      this.DATA_TOGGLE = '[data-select="toggle"]';
      this._el = typeof target === 'string' ? document.querySelector(target) : target;
      this._params = params || {};
      this._onClickFn = this._onClick.bind(this);
      if (this._params.options) {
        this._el.innerHTML = this.template(this._params);
        this._el.classList.add(this.EL);
      }
      this._elToggle = this._el.querySelector(this.DATA_TOGGLE);
    ...
    _createClass(ItcCustomSelect, [{
      ...
      value: function _onClick(e) {
        var target = e.target;
        var type = target.closest(this.DATA).dataset.select;
        if (type === 'toggle') {
          this.toggle();
        } else if (type === 'option') {
      ...
      value: function _updateOption(el) {
        var elOption = el.closest(".".concat(this.EL_OPTION));
        var elOptionSel = this._el.querySelector(".".concat(this.EL_OPTION_SELECTED));
        if (elOptionSel) {
          elOptionSel.classList.remove(this.EL_OPTION_SELECTED);
        }
        elOption.classList.add(this.EL_OPTION_SELECTED);
      ...
      value: function _reset() {
        var selected = this._el.querySelector(".".concat(this.EL_OPTION_SELECTED));
        if (selected) {
          selected.classList.remove(this.EL_OPTION_SELECTED);
        }
      ...
      value: function _changeValue(el) {
        if (el.classList.contains(this.EL_OPTION_SELECTED)) {
          return;
        }
      ...
      value: function show() {
        var _this2 = this;
        document.querySelectorAll(this.EL_SHOW).forEach(function (el) {
          el.classList.remove(_this2.EL_SHOW);
        });
        this._el.classList.add("".concat(this.EL_SHOW));
      }
    }, {
      key: "hide",
      value: function hide() {
        this._el.classList.remove(this.EL_SHOW);
      }
    }, {
      key: "toggle",
      value: function toggle() {
        this._el.classList.contains(this.EL_SHOW) ? this.hide() : this.show();
      }
    }, {
      key: "dispose",
      ...
      value: function template(params) {
        ...
        options.forEach(function (option, index) {
          ...
          if (option[0] === targetValue) {
            selectedClass = " ".concat(_this4.EL_OPTION_SELECTED);
            selectedIndex = index;
      ...
      value: function hideOpenSelect() {
        ...
        document.addEventListener('click', function (e) {
          if (!e.target.closest(".".concat(_this5.EL))) {
            var elsActive = document.querySelectorAll(".".concat(_this5.EL_SHOW));
            elsActive.forEach(function (el) {
              el.classList.remove(_this5.EL_SHOW);
            });
          }
        });
      }
  ...
  function initSliders() {
    ...
    if (document.querySelector('.rs-slider__slider')) {
      new Swiper('.rs-slider__slider', {
        ...
        pagination: {
          el: '.rs-slider__paging .slider-paging',
          type: 'bullets',
          clickable: true
        },
        on: {
  ...
  ***************************************************************************** */
  ...
  function gsapAnimations() {
    var numbersEl = gsap.utils.toArray('.numbers-item');
    if (numbersEl) {
      var mm = gsap.matchMedia();
      mm.add('(min-width: 991.98px)', function () {
        numbersEl.forEach(function (el) {
          gsap.from(el, {
            scrollTrigger: {
              trigger: el,
              start: 'top bottom',
    ...
    // const animEl = document.querySelector('.rs-video__video:not(.mobile)');
    // const animWrap = document.querySelector('.rs-video__wrapper');
    // const dy = animWrap.querySelector('.section-title').getBoundingClientRect().height + 150;

    // if (animEl) {
    // 	let mm = gsap.matchMedia();
    // 	mm.add('(min-width: 991.98px)', () => {
    // 		gsap.set(animWrap, {height: `calc(100vh + ${dy}px)`});
    // 		gsap.to(animEl, {
    // 			scrollTrigger: {
    // 				trigger: animEl,
    // 				start: 'center bottom',
    ...
    // 			width: '100vw',
    // 			height: '100vh',
    // 			x: () => -animEl.getBoundingClientRect().x - 1,
    // 			y: dy,
    // 		});
    // 	});
    // }
  ...
  window.addEventListener('load', function (e) {
    ...
    var itcCustomSelects = document.querySelectorAll('.itc-select');
    var itsSelect = [];
    itcCustomSelects.forEach(function (node, index) {
      itsSelect[index] = new ItcCustomSelect(node);
    });

    /////////////////////////////////////////////////////////////////
    //video control////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////
    var muteBtn = document.querySelector('.rs-video__video button.mute');
    var playBtn = document.querySelector('.rs-video__video button.play');
    var video = document.querySelector('.rs-video__video:not(.mobile) video');
    var mobileVideo = document.querySelector('.rs-video__video.mobile video');
    if (muteBtn) {
    ...
    //form validate//////////////////////////////////////////////
    /////////////////////////////////////////////////////////////
    jQuery.validator.addMethod('fileSize', function (value, element, parametr) {
      var valid = true;
      if (element.files.length === 0) return true;
      var _iterator2 = _createForOfIteratorHelper(element.files),
        _step2;
      try {
        for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
          var file = _step2.value;
          if (file.size > parametr) valid = false;
        }
      } catch (err) {
        _iterator2.e(err);
      } finally {
      ...
      return valid;
    }, 'Выберите файл допустимого размера');
    if ($('#fosForm').length > 0) {
      $('#fosForm').validate({
        ...
        messages: {
          ...
          fosFile: {
            accept: 'Выберите файл допустимого типа'
          }
        }
      });
    }
