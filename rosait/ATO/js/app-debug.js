...
(function (factory) {
  ...
  function documentScroll(e) {
    ...
    if (scrolled > 267 && scrolled > scrollPrev) {
  ...
  function effectVirtualTransitionEnd(_ref9) {
    var swiper = _ref9.swiper,
        duration = _ref9.duration,
        transformEl = _ref9.transformEl,
        allSlides = _ref9.allSlides;
    var slides = swiper.slides,
        activeIndex = swiper.activeIndex,
        $wrapperEl = swiper.$wrapperEl;
  ...
  */
  ...
  function initSliders() {
      ...
      new Swiper('.rs-slider__slider', {
        ...
        pagination: {
          ...
          renderBullet: function renderBullet(index, className) {
            ...
            return "<span class=\"".concat(className, "\">").concat(index, "</span>");
          }
        },
        on: {
    ...
    if (document.querySelector('.rs-favorites__slider')) {
      var addZero = function addZero(num) {
        return num > 9 ? num : '0' + num;
      };

      var pagging = document.querySelector('.rs-favorites .pagging');
      var arrowPrev = document.querySelector('.rs-favorites .arrow_prev');
      var arrowNext = document.querySelector('.rs-favorites .arrow_next');
      new Swiper('.rs-favorites__slider', {
    ...
    if (document.querySelector('.rs-sertificates__slider')) {
      var _arrowPrev = document.querySelector('.rs-sertificates .arrow_prev');

      var _arrowNext = document.querySelector('.rs-sertificates .arrow_next');

      new Swiper('.rs-sertificates__slider', {
        modules: [Navigation],
        observer: true,
        observeParents: true,
        slidesPerView: 1,
  ...
  window.onload = function () {
    //tabs();
    // change order of advantages item
    var mql = window.matchMedia('(min-width: 991.98px)');
    var advItems = document.querySelectorAll('.rs-advantages__item');
    var count = advItems.length;
    var firstCount = Math.ceil(count / 2);
    var secondCount = Math.floor(count / 2);

    function screenTest(event) {
      if (event.matches) {
        for (var i = 0; i < firstCount; i++) {
          advItems[i].style.order = i + (i + 1);
        }

        for (var _i8 = 0; _i8 < secondCount; _i8++) {
          advItems[_i8 + firstCount].style.order = _i8 + (_i8 + 2);
          advItems[_i8 + firstCount].style.top = '100px';
        }
      } else {
        for (var _i9 = 0; _i9 < count; _i9++) {
          advItems[_i9].style.order = null;
          advItems[_i9].style.top = null;
        }
      }
    }

    screenTest(mql);
    mql.addEventListener('change', screenTest); //hover design items

    $('.design-item').on('mouseenter', function (e) {
