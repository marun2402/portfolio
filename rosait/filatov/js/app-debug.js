...
(function (factory) {
  ...
  */
  ...
  function initSliders() {
    ...
    if (gallerySlider) {
      var _slider = gallerySlider.querySelector('[class*="__slider"]');

      var arrowNext = gallerySlider.querySelector('[class*="_next"]');
      var arrowPrev = gallerySlider.querySelector('[class*="_prev"]'); //const pagging = gallerySlider.querySelector('[class*="__paging"]');
    ...
    if (document.querySelector('.rs-catalog')) {
      var disableEnableSlider = function disableEnableSlider(evt) {
        if (evt.matches) {
          if (catalogtSlider !== undefined) {
            catalogtSlider.destroy(true, true);
          }
        } else {
          catalogtSlider = new Swiper('.rs-catalog__content', {
            modules: [Navigation],
            slidesPerView: 2,
            autoHeight: false,
            spaceBetween: 30,
            ...
            loop: false,
            navigation: {
              nextEl: '.rs-catalog__arrows .slider-arrow_next',
              prevEl: '.rs-catalog__arrows .slider-arrow_prev'
            },
            breakpoints: {
              992: {
                slidesPerView: 3
  ...
  var slider = document.querySelector('.rs-slider');
  ...
  function updateMe() {
    ...
    var degree = radius * 12; // gsap.to('.banner__photos img:nth-child(3), .banner__photos img:nth-child(4)', 1, {
    // 	transform: `translate(${dx * 0.015}px, ${dy * 0.015}px)`,
    // });
    // gsap.to('.banner__photos img:nth-child(1), .banner__photos img:nth-child(2)', 1, {
    // 	transform: `translate(${dx * -0.01}px, ${dy * -0.01}px)`,
    // });

    gsap.to('.banner__photos', 1, {
  ...
  function countdownTimer() {
    ...
    var time = timer.dataset.deadline;
    var deadline;

    if (!time) {
      deadline = new Date();
    } else {
      deadline = new Date(parseInt(time.split('.')[2]), parseInt(time.split('.')[1]) - 1, parseInt(time.split('.')[0]));
    } // id таймера


    var timerId = null; // получаем элементы, содержащие компоненты даты

    var $days = timer.querySelector('.timer__days');
    var $hours = timer.querySelector('.timer__hours');
    var $minutes = timer.querySelector('.timer__minutes');
    var $seconds = timer.querySelector('.timer__seconds'); // вызываем функцию countdownTimerTick

    countdownTimerTick(deadline, timerId, $days, $hours, $minutes, $seconds); // вызываем функцию countdownTimerTick каждую секунду

    timerId = setInterval(function () {
      countdownTimerTick(deadline, timerId, $days, $hours, $minutes, $seconds);
    }, 1000);
  ...
  function countdownTimerTick(deadline, timerId, $days, $hours, $minutes, $seconds) {
