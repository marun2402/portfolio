...
(function (factory) {
  ...
  if (spollersArray.length > 0) {
    ...
    var hideSpollersBody = function hideSpollersBody(spollersBlock) {
      ...
      if (spollerActiveTitle) {
        ...
        _slideUp$1(spollerActiveTitle.nextElementSibling, 500);
      }
    };
  ...
  var _slideUp$1 = function _slideUp$1(target) {
  ...
  var _slideDown$1 = function _slideDown$1(target) {
    var duration = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 500;
  ...
  var anchors = document.querySelectorAll('.anchor-links a[href*="#"]');
  ...
  */
    ...
    function setTabsAction(e) {
      ...
      if (el.closest('[data-tabs-title]')) {
        ...
        if (!tabTitle.classList.contains('_tab-active') && !tabsBlock.querySelectorAll('._slide').length) {
          var tabActiveTitle = tabsBlock.querySelector('[data-tabs-title]._tab-active');
  ...
  function dataMediaQueries(array, dataSetValue) {
    ...
    if (media.length) {
      ...
      media.forEach(function (item) {
        ...
        _breakpointsArray.push(breakpoint);
      }); // Получаем уникальные брейкпоинты

      var mdQueries = _breakpointsArray.map(function (item) {
        return '(' + item.type + "-width: " + item.value + "px)," + item.value + ',' + item.type;
      });
      ...
      if (mdQueries.length) {
        ...
        mdQueries.forEach(function (breakpoint) {
          ...
          var itemsArray = _breakpointsArray.filter(function (item) {
            if (item.value === mediaBreakpoint && item.type === mediaType) {
              return true;
            }
          });
  ...
  function loopCreate() {
    ...
    for (var _i4 = 0; _i4 < appendSlides.length; _i4 += 1) {
      $selector.append($(appendSlides[_i4].cloneNode(true)).addClass(params.slideDuplicateClass));
    }

    for (var _i5 = prependSlides.length - 1; _i5 >= 0; _i5 -= 1) {
      $selector.prepend($(prependSlides[_i5].cloneNode(true)).addClass(params.slideDuplicateClass));
    }
  }
  ...
  */
  ...
  function initSliders() {
    ...
    if (document.querySelector('.rs-slider__slider')) {
      new Swiper('.rs-slider__slider', {
        ...
        // Dotts
        pagination: {
          el: '.rs-slider__pagging',
          clickable: true,
          renderBullet: function renderBullet(index, className) {
            var slider = document.querySelector('.rs-slider__slider');
            var slides = slider.querySelectorAll('.rs-slider__slide:not(.swiper-slide-duplicate)');
            var name = slides[index].querySelector('.rs-slider__slide-title').innerHTML;
            var src = slides[index].dataset.src;
            return "<div class=\"".concat(className, "\">\n\t\t\t\t\t\t\t\t\t\t<span class=\"").concat(className, "__image-ibg\">\n\t\t\t\t\t\t\t\t\t\t\t<img src=\"").concat(src, "\" alt=\"\">\n\t\t\t\t\t\t\t\t\t\t</span>\n\t\t\t\t\t\t\t\t\t\t<span class=\"").concat(className, "__name\">").concat(name, "</span>\n\t\t\t\t\t\t\t\t\t</div>");
          }
        },
        on: {
    ...
    if (document.querySelector('.rs-news__slider')) {
      new Swiper('.rs-news__slider', {
        ...
        // Arrows
        navigation: {
          nextEl: '.rs-news__arrow_next',
          prevEl: '.rs-news__arrow_prev'
        },
        breakpoints: {
          768: {
            slidesPerView: 2
    ...
    var contentSliders = document.querySelectorAll('.js-content-slider');
    contentSliders.forEach(function (node, _) {
      new contentSlider(node);
    }); //====================================================================================================================

    if (document.querySelector('.partners-slider__slider')) {
      new Swiper('.partners-slider__slider', {
        modules: [Navigation, Pagination],
        observer: true,
        observeParents: true,
        slidesPerView: 1,
  ...
  var contentSlider = /*#__PURE__*/function () {
    function contentSlider($component) {
  ...
  ***************************************************************************** */
  ...
  window.onload = function () {
    tabs(); //dropdown menu resizer

    var menu = document.querySelector('.menu');

    if (menu) {
      dropdownMenuResizer();
      window.addEventListener('resize', dropdownMenuResizer);
    }

    function dropdownMenuResizer() {
      var dropWidth = document.querySelector('header').clientWidth;
      var menuX = menu.getBoundingClientRect().x;
      menu.style.setProperty('--drop-width', dropWidth + 'px');
      menu.style.setProperty('--submenu-left', menuX + 'px');
      var lists = menu.querySelectorAll('.menu__dropdown-list');

      var listsHeight = _toConsumableArray(lists).map(function (list) {
        return list.offsetHeight;
      });

      var maxHeight = Math.max.apply(null, listsHeight);
      menu.style.setProperty('--submenu-height', maxHeight + 'px');
    } //events handling


    var isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    document.addEventListener('click', clickActions); // sticky navigate this anchor links

    var navigate = document.querySelector('.rs-product__navigate');
    var offset = 0;

    if (navigate) {
      offset = navigate.offsetTop;
