  ...
  function documentActions(e) {
    ...
    if (window.innerWidth < 992) {
      if (targetElement.closest('.mobile-menu-button')) {
        ...
        document.querySelector('.header-top').classList.toggle('_active');
      }
    }
    //open-close search button
    if (targetElement.closest('.search-btn')) {
      const btn = targetElement.closest('.search-btn');
      btn.classList.toggle('_open');
      targetElement.closest('.search').classList.toggle('_open');
      if (btn.firstChild.classList.contains('fa-search')) {
        btn.firstChild.className = 'fa fa-times';
      } else {
        btn.firstChild.className = 'fa fa-search';
      }
    }
  }
}; //.window-onload
