...
(function (factory) {
  ...
  function checkViewport() {
    ...
    var elements2 = gsap.utils.toArray('.rs-banner');
  ...
  function initSliders() {
    ...
    if (document.querySelector('.feedback-slider')) {
      var disableEnableSlider = function disableEnableSlider(evt) {
        if (evt.matches) {
          if (feedbackSlider !== undefined) {
            feedbackSlider.destroy(true, true);
          }
        } else {
          feedbackSlider = new Swiper('.feedback-slider__slider', {
            slidesPerView: 1,
            speed: 800,
            loop: false,
            spaceBetween: 0,
            ...
            navigation: {
              nextEl: '.feedback-slider__nav .slider-arrow_next',
              prevEl: '.feedback-slider__nav .slider-arrow_prev'
            }
          });
        }
      };
  ...
  ***************************************************************************** */
  ...
  function gsapAnimations() {
    var tiltElements = gsap.utils.toArray('.tilt');
    tiltElements.forEach(function (node, _) {
      new tiltElement(node);
    });
    var animEl = document.querySelector('.rs-video__video');
    if (animEl) {
      ...
      mm.add('(min-width: 767.98px)', function () {
        gsap.to(animEl, {
          ...
          height: '100vh',
          x: function x() {
            return -animEl.getBoundingClientRect().x - 1;
          }
        });
      });
    }
  }
  var tiltElement = /*#__PURE__*/function () {
    function tiltElement(elem) {
      _classCallCheck(this, tiltElement);
      this.elem = elem;
    ...
    _createClass(tiltElement, [{
      key: "addHandlers",
      value: function addHandlers() {
        var _this2 = this;
        this.elem.addEventListener('mousemove', function (event) {
          _this2.elemX = event.clientX - _this2.elem.getBoundingClientRect().left;
          _this2.elemY = event.clientY - _this2.elem.getBoundingClientRect().top;
          requestAnimationFrame(_this2.update.bind(_this2));
        });
      ...
      value: function update() {
        ...
        gsap.to(this.elem, 0.2, {
          transform: "translate(".concat(this.dx * -0.08, "px, ").concat(this.dy * -0.2, "px)")
        });
      }
    }, {
      key: "reset",
  ...
  window.addEventListener('load', function (e) {
    ...
    //line and title banner animation
    var bannerLine = document.querySelector('.rs-banner #line path');
    var letters = document.querySelector('.rs-banner #letters');
    var lettersWrapper = document.querySelector('.rs-banner #lettersWrapper');
    if (bannerLine) {
      var lineLength = bannerLine.getTotalLength();
      gsap.set(bannerLine, {
    ...
    if ($('#fosForm').length > 0) {
      $('#fosForm').validate({
        ...
        messages: {
          ...
          fosContact: {
            required: 'Введите телефон или почту'
          },
          fosMessage: {
