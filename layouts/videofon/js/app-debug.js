  ...
  function initSliders() {
    ...
    if ($servicesSlider) {
      var disableEnableSlider = function disableEnableSlider(evt) {
        if (evt.matches) {
          if (servicesSlider !== undefined) {
            servicesSlider.destroy(true, true);
          }
        } else {
          var $counter = $servicesSlider.querySelector('.services-slider__counter');
          var counter = function counter(swiper) {
            var currentSlide = swiper.activeIndex + 1;
            var totalSlides = swiper.slides.length;
            $counter.innerHTML = "<span class=\"current\">".concat(currentSlide < 10 ? '0' + currentSlide : currentSlide, "</span>\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t <span class=\"total\"> / ").concat(totalSlides < 10 ? '0' + totalSlides : totalSlides, "</span> ");
          };
          servicesSlider = new Swiper(slider, {
            slidesPerView: 1,
            spaceBetween: 25,
            autoHeight: false,
            speed: 800,
      ...
      var slider = $servicesSlider.querySelector('[class*="__slider"]');
      var pagging = $servicesSlider.querySelector('[class*="__paging"]');
      var mediaQueryList = window.matchMedia('(max-width: 991.98px)');
  ...
  ***************************************************************************** */
  ...
  function customCursor() {
    if (document.querySelector('.custom-cursor')) {
      var y = $('[data-cursor]').offset().top + $('[data-cursor]').height();
      $('[data-cursor]').hover(function () {
        $('.custom-cursor').addClass('active');
        $('body').addClass('is-show-cursor');
      }, function () {
        $('.custom-cursor').removeClass('active');
        $('body').removeClass('is-show-cursor');
      });
      gsap.set('.custom-cursor', {
        xPercent: -50,
        yPercent: -50
      });
      var ball = document.querySelector('.custom-cursor');
      var pos = {
